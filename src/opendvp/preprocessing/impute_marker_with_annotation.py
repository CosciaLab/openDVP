...
def negate_var_by_ann(adata, target_variable, target_annotation_column , quantile_for_imputation=0.05) -> ad.AnnData:
    ...
    assert target_annotation_column in adata.obs.columns, (
        f"Annotation column {target_annotation_column} not found in adata.obs.columns"
    )
    ...
    value_to_impute = np.quantile(
        adata_copy[:, target_var_idx].X.toarray(), quantile_for_imputation
    )
    logger.info(
